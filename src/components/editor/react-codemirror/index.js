    ...
    Object.keys(props || {})
      ...
      .forEach(function (prop) {
        switch (prop) {
          ...
          case 'onCursor':
            {
              _this.editor.on('cursorActivity', function (cm) {
                _this.props.onCursor(
                  _this.editor,
                  _this.editor.getDoc().getCursor()
                );
              });
            }
            break;
          ...
          case 'onGutterClick':
            {
              _this.editor.on('gutterClick', function (
                cm,
                lineNumber,
                gutter,
                event
              ) {
                _this.props.onGutterClick(
                  _this.editor,
                  lineNumber,
                  gutter,
                  event
...
var Controlled = (function (_super) {
  ...
  Controlled.prototype.hydrate = function (props) {
    ...
    var userDefinedOptions = _extends(
      {},
      cm.defaults,
      this.editor.options,
      _options
  ...
  Controlled.prototype.componentDidMount = function () {
    ...
    if (this.props.defineMode) {
      this.props.defineMode.map((df) => {
        if (df.name && df.fn) {
          cm.defineMode(df.name, df.fn);
        }
      });
    }
...
var UnControlled = (function (_super) {
  ...
  UnControlled.prototype.componentDidMount = function () {
    ...
    this.editor.on('beforeChange', function (cm, data) {
      if (_this.props.onBeforeChange) {
        _this.props.onBeforeChange(
          _this.editor,
          data,
          _this.editor.getValue(),
          _this.onBeforeChangeCb
        );
      }
    });
    this.editor.on('change', function (cm, data) {
